...
import { FileSpreadsheet, CheckCircle, AlertCircle, Loader2, ExternalLink } from 'lucide-react'

interface SheetsExportProps {
  itinerary: Itinerary
}

export function SheetsExport({ itinerary }: SheetsExportProps) {
  const { exportToSheets, isExporting, error } = useSheets()
  const [exportedUrl, setExportedUrl] = useState<string | null>(null)

  const handleExport = async () => {
    try {
      const sheetUrl = await exportToSheets(itinerary)
    ...
    <div className="border border-gray-200 rounded-lg p-4 hover:border-indigo-300 transition-all duration-200">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-base font-semibold text-gray-900 mb-1">Google Sheets</h4>
          <p className="text-sm text-gray-600 mb-3">
            Export detailed budget breakdown with charts and daily expenses
          </p>
          ...
          {exportedUrl && (
            <div className="flex items-start gap-2 p-3 bg-green-50 border border-green-200 rounded-lg mb-3">
              ...
              <div className="flex-1">
                <p className="text-sm text-green-700 mb-2">Successfully exported to spreadsheet!</p>
                <a
                  href={exportedUrl}
                  target="_blank"
